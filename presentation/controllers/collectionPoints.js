const service = require('../../service/collectionPoint.js');

exports.getAll = async (req, res) => {
  try {
    const collectionPoints = await service.getAll();
...
exports.getById = async (req, res) => {
  try {
    const collectionPoint = await service.getById(req.params.id);
...
exports.delete = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Collection Point is deleted successfully' });
  } catch (err) {
    console.error(err);
